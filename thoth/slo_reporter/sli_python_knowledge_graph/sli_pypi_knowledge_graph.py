...
class SLIPyPIKnowledgeGraph(SLIBase):
    ...
    def _query_sli(self) -> Dict[str, Any]:
        ...
        query_labels_packages = f'{{field="{self.configuration.instance}", stats_type="packages", job="metrics-exporter"}}'
        query_labels_releases = f'{{field="{self.configuration.instance}", stats_type="releases", job="metrics-exporter"}}'
